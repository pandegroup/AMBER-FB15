...
RTP = '/home/leeping/opt/gromacs/share/gromacs/top/amberfb15.ff/aminoacids.rtp'
...
def ParseRTP(rtp):
    for line in open(rtp).readlines():
        ...
        if re.match('\[ .* \]$', line):
            section = s[1]
            # Amino acid names are "sections" in the .rtp file.
    ...
    if force.tag == 'PeriodicTorsionForce':
        ...
        for elem in force:
            for i in range(1, 7):
                ...
                k = 'k%i' % i
                if p in elem.attrib:
                    if np.abs(float(elem.attrib[f])) < 1e-3 and np.abs(float(elem.attrib[k])) < 1e-3:
                        # print "DELETING", p, elem.attrib[f], elem.attrib[k]
                        # del elem.attrib[p]
                        # del elem.attrib[f]
                        # del elem.attrib[k]
                        elem.attrib[f] = '0.0'
                        elem.attrib[k] = '0.0'
                    elif np.abs(float(elem.attrib[f])) < 1e-3:
                        elem.attrib[f] = '0.0'
            if all(['periodicity%i' % i not in elem.attrib for i in range(1,7)]):
                print "ZERO INTERACTION", elem.attrib
                elem.attrib['periodicity1'] = '1'
                elem.attrib['phase1'] = '0.0'
                elem.attrib['k1'] = '0.0'
            else:
                print "KEEP INTERACTION", elem.attrib
